...
class TradingAlgorithm:
    def __init__(self, high_spread_threshold=1.35, direction_wait_ticks=130, max_position=100, rolling_window=50):
        ...
        self.sma83_deque = deque(maxlen=83)
        self.sum_sma83 = 0.0
    ...
    def getOrders(self, current_data: Dict[str, Dict[str, float]], order_data: Dict[str, int]) -> Dict[str, int]:
        ...
        sma83 = self.updateSMA83(mid_now)
        ...
        # Early exit logic based on SMA83 trend reversal
        if self.state == "IN_POSITION" and len(self.sma83_deque) == 83:
            prev_sma = list(self.sma83_deque)[0]
            current_sma = sma83
            if (pos_now > 0 and current_sma < prev_sma) or (pos_now < 0 and current_sma > prev_sma):
                qty_close = -pos_now
                order_data[self.product] = qty_close
                self._recordTrade(qty_close, ask if qty_close > 0 else bid, 'early_close')
                self.state = "FLAT"
    ...
    def updateRollingAvg(self, new_mid_price: float) -> float:
        if len(self.mid_deque) == self.rolling_window:
            self.sum_mid -= self.mid_deque[0]
        self.mid_deque.append(new_mid_price)
        self.sum_mid += new_mid_price
        return self.sum_mid / len(self.mid_deque)

    def updateSMA83(self, new_mid_price: float) -> float:
        if len(self.sma83_deque) == self.sma83_deque.maxlen:
            self.sum_sma83 -= self.sma83_deque[0]
        self.sma83_deque.append(new_mid_price)
        self.sum_sma83 += new_mid_price
        return self.sum_sma83 / len(self.sma83_deque) if self.sma83_deque else new_mid_price
